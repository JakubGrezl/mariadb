...
--
-- Host: db
-- Generation Time: Nov 27, 2023 at 10:24 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`%` PROCEDURE `HodKostkou` (`maxcislo` INT)   BEGIN
  SELECT CONCAT("Hodil jsi číslo", FLOOR(RAND() * maxcislo) + 1);
END$$
...
--
-- Table structure for table `adresy_faktur`
--
...
--
-- Dumping data for table `adresy_faktur`
--
...
(10022, 'Novaokna s r.o.', 2147483647, 'CZ3456789012', 'Husitská 9, Krupka'),
(10037, 'Motospeed-MK', 98764321, 'CZ98764321', 'Na vršku 3, Proboštov');

-- --------------------------------------------------------

--
-- Table structure for table `druhy_vyrobku`
--
...
--
-- Dumping data for table `druhy_vyrobku`
--
...
INSERT INTO `mesta` (`id_mesta`, `nazev`) VALUES
(1, 'Ústí nad Labem'),
(2, 'Teplice'),
(3, 'Děčín'),
(4, 'Most'),
...
CREATE TABLE `objednavky` (
  ...
  `zam_zpr` int(11) DEFAULT NULL,
  `dat_exp` datetime DEFAULT NULL,
  `zam_exp` int(11) DEFAULT NULL,
  `adr_dor` varchar(50) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_czech_ci;
...
--
-- Table structure for table `obj_vyr`
--

CREATE TABLE `obj_vyr` (
  `cislo` int(11) NOT NULL,
  `id_vyr` int(11) NOT NULL,
  `kusy` tinyint(4) NOT NULL DEFAULT 1
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_czech_ci;

--
-- Dumping data for table `obj_vyr`
--
...
CREATE TABLE `pozice` (
  ...
  `id_poz_nad` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_czech_ci;
...
(203, 'Red Bull', 35.00, 5, 'Energy Drink osvěží tělo i mysl, zvýší výkon, soustředění, bdělost a rychlost reakce, 250m'),
...
(300, 'Krušovice Ležák 12', 32.00, 6, 'světlý ležák, alk. 5,0 % obj., 500ml'),
(301, 'Krušovice Černé', 30.00, 6, 'tmavé pivo, alk. 3,8 % obj., 500ml'),
(302, 'Krušovice Mušketýr 11', 31.00, 6, 'výčepní tmavé pivo, alk. 4,5 % obj., 500ml'),
(303, 'Gambrinus Originál 10', 28.00, 6, 'výčepní světlé pivo, alk. 4,3 % obj., 500ml'),
(304, 'Gambrinus Patron 12', 32.00, 6, 'světlý ležák, alk. 5,2 % obj., 500ml'),
(305, 'Pilsner Urquell', 35.00, 6, 'světlý ležák, alk. 4,4 % obj., 500ml'),
(350, 'Müller Thurgau', 185.00, 7, 'jakostní kabinetní víno s přívlastkem, polosuché, 750ml'),
(351, 'Chardonnay', 270.00, 7, 'pozdní sběr, suché, 750ml'),
(360, 'Modrý Portugal', 109.00, 7, 'České zemské víno, suché, 750ml'),
(361, 'Svatovavřinecké', 179.00, 7, 'České zemské víno, suché, 750ml');
...
--
-- Indexes for table `adresy_faktur`
--
ALTER TABLE `adresy_faktur`
  ADD PRIMARY KEY (`cislo`);

--
-- Indexes for table `druhy_vyrobku`
--
ALTER TABLE `druhy_vyrobku`
  ADD PRIMARY KEY (`id_dru`);
...
ALTER TABLE `objednavky`
  ADD PRIMARY KEY (`cislo`),
  ADD KEY `fk_zakaznik` (`id_zak`),
  ADD KEY `fk_zak` (`zam_pri`),
  ADD KEY `fk_zam` (`zam_zpr`),
  ADD KEY `fk_zame` (`zam_exp`);

--
-- Indexes for table `obj_vyr`
--
ALTER TABLE `obj_vyr`
  ADD KEY `fk_vyr` (`id_vyr`),
  ADD KEY `fk_objednavky` (`cislo`);
...
ALTER TABLE `pozice`
  ...
  ADD KEY `fk_nadrizeny` (`id_poz_nad`);

--
-- Indexes for table `vyrobky`
--
ALTER TABLE `vyrobky`
  ADD PRIMARY KEY (`id_vyr`),
  ADD KEY `fk_druhy_vyr` (`id_dru`);
...
--
ALTER TABLE `mesta`
  MODIFY `id_mesta` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10;

--
-- AUTO_INCREMENT for table `objednavky`
--
ALTER TABLE `objednavky`
  MODIFY `cislo` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10053;

--
-- AUTO_INCREMENT for table `pozice`
--
ALTER TABLE `pozice`
  MODIFY `id_poz` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
...
--
-- Constraints for table `adresy_faktur`
--
ALTER TABLE `adresy_faktur`
  ADD CONSTRAINT `fk_cislo` FOREIGN KEY (`cislo`) REFERENCES `objednavky` (`cislo`);

--
-- Constraints for table `objednavky`
--
ALTER TABLE `objednavky`
  ADD CONSTRAINT `fk_zak` FOREIGN KEY (`zam_pri`) REFERENCES `zamestnanci` (`id_zam`),
  ADD CONSTRAINT `fk_zakaznik` FOREIGN KEY (`id_zak`) REFERENCES `zakaznici` (`id_zak`),
  ADD CONSTRAINT `fk_zam` FOREIGN KEY (`zam_zpr`) REFERENCES `zamestnanci` (`id_zam`),
  ADD CONSTRAINT `fk_zame` FOREIGN KEY (`zam_exp`) REFERENCES `zamestnanci` (`id_zam`);

--
-- Constraints for table `obj_vyr`
--
ALTER TABLE `obj_vyr`
  ADD CONSTRAINT `fk_objednavky` FOREIGN KEY (`cislo`) REFERENCES `objednavky` (`cislo`),
  ADD CONSTRAINT `fk_vyr` FOREIGN KEY (`id_vyr`) REFERENCES `vyrobky` (`id_vyr`);

--
-- Constraints for table `pozice`
--
ALTER TABLE `pozice`
  ADD CONSTRAINT `fk_nadrizeny` FOREIGN KEY (`id_poz_nad`) REFERENCES `pozice` (`id_poz`);

--
-- Constraints for table `vyrobky`
--
ALTER TABLE `vyrobky`
  ADD CONSTRAINT `fk_druhy_vyr` FOREIGN KEY (`id_dru`) REFERENCES `druhy_vyrobku` (`id_dru`);

--
-- Constraints for table `zakaznici`
--
ALTER TABLE `zakaznici`
  ADD CONSTRAINT `fk_mesta` FOREIGN KEY (`id_mes`) REFERENCES `mesta` (`id_mesta`);

--
-- Constraints for table `zamestnanci`
--
ALTER TABLE `zamestnanci`
  ADD CONSTRAINT `fk_pozice` FOREIGN KEY (`id_poz`) REFERENCES `pozice` (`id_poz`);
COMMIT;
